...
### Requirement: DoH Unit Detail View (previous version)
**Reason**: Replacing separate page route with Sheet overlay for better comparison workflow and pattern consistency.

**Migration**: The separate page route `/dashboard/units/[id]` may be kept for backward compatibility or deep linking needs, but the primary interaction is now through the Sheet overlay. Any direct links should be updated to trigger the sheet instead.
