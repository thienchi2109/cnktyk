# activities-catalog Specification

## Purpose
The activities catalog provides SoYTe and DonVi operators with a unified table for browsing, searching, and filtering all CPD activities. It ensures the UI always reflects authoritative server data, supports large datasets through server-driven pagination, and keeps filter/search state synchronized with URL parameters for shareable views.
...
- **WHEN** the user changes page size from 25 to 50
- **THEN** the UI resets `page=1` and re-requests `/api/activities?...&limit=50`
