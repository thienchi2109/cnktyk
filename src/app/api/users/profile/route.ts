...
export async function GET() {
  try {
    ...
    const user = await taiKhoanRepo.findById(session.sub);
    if (!user) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (validatedData.MatKhau) {
      await taiKhoanRepo.updatePassword(session.sub, validatedData.MatKhau);
    }

    // Get updated user data
    const updatedUser = await taiKhoanRepo.findById(session.sub);
    if (!updatedUser) {
      return NextResponse.json({ error: 'User not found' }, { status: 404 });
    }
